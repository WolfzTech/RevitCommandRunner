...
# Step 1: Create temporary staging directory with bundle
...
# Step 3: Create ZIP of bundle (include PackageContents.xml at root for installer)
Write-Host "Step 3: Creating bundle ZIP..." -ForegroundColor Yellow
$bundleZip = Join-Path $stagingDir "bundle.zip"

# The ZIP should contain RevitCommandRunner.bundle folder
# When extracted to ApplicationPlugins, it will create ApplicationPlugins/RevitCommandRunner.bundle/
...
# Step 4: Embed ZIP as resource in installer project
...
# Update .csproj to include embedded resource
...
if ($csproj -notmatch '<EmbeddedResource Include="Resources\\bundle.zip"') {
    Write-Host "  Adding embedded resource to .csproj..." -ForegroundColor Gray
    
    # Add ItemGroup with EmbeddedResource before closing </Project>
    $resourceXml = @"
  <ItemGroup>
...
# Step 5: Build installer with embedded resource
...
# Remove embedded resource from .csproj (keep it clean for next build)
$csproj = Get-Content $csprojPath -Raw
